    ...
    <div class="flex flex-col lg:flex-row gap-5">
        <div class="bg-neutral flex flex-col border-back rounded-xl w-full p-5 sm:p-7">
            ...
            <form method="POST" action="{{ route('update.theme_setting') }}" enctype="multipart/form-data" class="mt-8">
                @csrf
                <!-- General Info -->
                <h2 class="text-white font-semibold mb-4 border-b border-gray-700 pb-2">Informasi Klinik</h2>
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-8">
                    @foreach([
                        ...
                        <div class="flex flex-col gap-2 {{ $field == 'alamat' ? 'sm:col-span-2' : '' }}">
                            <label for="{{ $field }}" class="text-sm font-medium text-white">{{ $label }}</label>
                            @if($field == 'alamat')
                ...
                <!-- Assets / Uploads -->
                <h2 class="text-white font-semibold mb-4 border-b border-gray-700 pb-2">Logo & Favicon</h2>
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-6 mb-8">
                    @foreach(['logo' => 'Logo Utama', 'favicon' => 'Logo Favicon (Ikon atas website)'] as $field => $label)
                        <div class="flex flex-col gap-2">
                            ...
                            <span class="text-xs opacity-60 text-white">Biarkan kosong jika tidak ingin mengubah.</span>
                            @error($field) <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
                        </div>
                    @endforeach
                </div>
                ...
                <!-- Theme Colors -->
                <h2 class="text-white font-semibold mb-4 border-b border-gray-700 pb-2">Palet Warna (Theme Colors)</h2>
                <div class="grid grid-cols-2 sm:grid-cols-3 gap-4 mb-8">
                    ...
                    @foreach($colors as $key => $info)
                        <div class="flex gap-3 items-center bg-white/5 p-3 rounded-xl border border-white/10 hover:bg-white/10 transition-colors">
                            ...
                            <div class="flex flex-col">
                                <label for="{{ $key }}" class="text-xs font-semibold text-white uppercase">{{ $info['alias'] }}</label>
                                <span class="text-[10px] text-white/50 leading-tight">{{ $info['desc'] }}</span>
                            </div>
                        </div>
                    @endforeach
                </div>
                ...
                <div class="flex gap-3 justify-end mt-4 pt-4 border-t border-gray-700">
                    <button type="reset" class="btn border-none text-white hover:bg-neutral-focus">Reset</button>
                    <button type="submit" class="btn btn-primary border-none text-white hover:bg-[#c97433]">Simpan Pengaturan</button>
                </div>
            </form>
        </div>
    </div>
